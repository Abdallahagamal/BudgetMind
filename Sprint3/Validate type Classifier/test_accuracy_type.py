"""
Sprint 3 : TYPE classifier — Test Classification Accuracy

Purpose: confirm the classifier still performs as documented, in the current
environment, before it's relied on by the rest of the pipeline this sprint.
This is a regression check against Sprint 2's committed numbers
(Sprint2/results/type_results.json) — catches things like a scikit-learn
version drift silently changing predictions, a corrupted/re-saved model file,
or an accidental edit to the embeddings on disk.

Checks:
  1. Sprint 2 baseline reproduces its recorded RANDOM-protocol test metrics
     within tolerance.
  2. Sprint 3's refined (calibrated) model is evaluated the same way, for
     side-by-side comparison.
  3. Data integrity: embeddings are still 384-dim, L2-normalized, and the
     test set still has the same task_ids Sprint 2 evaluated on.
  4. Interface contract: both models still return {prediction, confidence,
     margin} correctly via predict_with_margin().
"""
